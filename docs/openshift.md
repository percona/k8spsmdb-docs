...
Percona Operator for Percona Server for MongoDB is a [Red Hat Certified Operator :material-arrow-top-right:](https://connect.redhat.com/en/partner-with-us/red-hat-openshift-certification). This means that Percona Operator is portable across hybrid clouds and fully supports the Red Hat OpenShift lifecycle.
...
You can install Percona Operator for MongoDB on OpenShift using the [Red Hat Marketplace :material-arrow-top-right:](https://marketplace.redhat.com) web interface or using the command line interface.

### Install the Operator via the Red Hat Marketplace

1. login to the Red Hat Marketplace and register your cluster [following the official instructions :material-arrow-top-right:](https://marketplace.redhat.com/en-us/workspace/clusters/add/register).
2. Go to the Percona Operator for MongoDB [page :material-arrow-top-right:](https://marketplace.redhat.com/en-us/products/percona-server-for-mongodb) and click the Free trial button:
...
2. The Custom Resource Definition for Percona Server for MongoDB should be
    ...
    If you have a [cert-manager :material-arrow-top-right:](https://docs.cert-manager.io/en/release-0.8/getting-started/install/openshift.html) installed, then you have to execute two more commands to be able to manage certificates with a non-privileged user:
...
4. Add role-based access control (RBAC) for Percona Server for MongoDB is
    ...
    and roles can be found in [OpenShift documentation :material-arrow-top-right:](https://docs.openshift.com/enterprise/3.0/architecture/additional_concepts/authorization.html).
...
1. Add the MongoDB Users secrets to OpenShift. These secrets
    ...
    documentation :material-arrow-top-right:](https://kubernetes.io/docs/concepts/configuration/secret/)
    for details).
