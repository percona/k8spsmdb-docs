...
* **[Restore from a backup](backups-restore.md)** — Restore on the same cluster or [provision a new cluster from a backup](backups-restore.md#restore-on-a-new-cluster).
* **[Monitor the database and Kubernetes](monitoring.md)** — Use PMM [to monitor the database](monitoring.md) and [Kubernetes](monitor-kubernetes.md) health and performance for full observability.
