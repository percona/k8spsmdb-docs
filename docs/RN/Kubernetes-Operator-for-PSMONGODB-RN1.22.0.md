...
* [K8SPSMDB-1548](https://perconadev.atlassian.net/browse/K8SPSMDB-1548) Added the ability to specify custom environment variables container for cluster components via the Custom Resource. This provides more granular control over container configurations.
...
* [K8SPSMDB-1448](https://perconadev.atlassian.net/browse/K8SPSMDB-1448) - Integrated HashiCorp Vault for system user credentials management, enabling you to storage and manage them in a centralized place. The Operator securely fetches and manages database passwords from a central Vault instance during cluster creation.
...
??? In progress * [K8SPSMDB-1413](https://perconadev.atlassian.net/browse/K8SPSMDB-1413) - Fixed a bug where `cert-manager` integration failed when utilizing the `issuerConf` configuration for custom certificate issuers. The Operator now correctly recognizes and processes custom issuer settings, ensuring seamless automated TLS certificate management and renewals in complex environments.
