...
Also, the [`psmdb-db` Helm chart](https://github.com/percona/percona-helm-charts/tree/main/charts/psmdb-db) undergoes rename of one parameter: starting from this release, the old parameter `users` that allows specifying [system users](../users.md#system-users) is renamed to `systemUsers`. Now parameter `users` is referring to the new [Declarative user management](../users.md#create-users-in-the-custom-resource).
