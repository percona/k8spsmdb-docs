...
| Backup type | Version added | Status | Description | Use this when | Important considerations |
| --- | --- | --- | --- | --- | --- |
| [Full logical :octicons-link-external-16:](https://docs.percona.com/percona-backup-mongodb/features/logical.html) | Initial | GA | Queries the database and writes the data to remote storage | You want portability or [selective restore](backups-restore.md#selective-restore) | Uses less storage but is slower than physical backups. Supports point-in-time recovery. Incompatible with backups made with Operator versions before 1.9.0 — take a new backup after upgrading. |
| [Full physical :octicons-link-external-16:](https://docs.percona.com/percona-backup-mongodb/features/physical.html) | [1.14.0](RN/Kubernetes-Operator-for-PSMONGODB-RN1.14.0.md) | GA ([1.16.0](RN/Kubernetes-Operator-for-PSMONGODB-RN1.16.0.md)) | Copies files from MongoDB `dbPath` to remote storage | You have a large dataset and need faster restore | Supports point-in-time recovery since [1.15.0](RN/Kubernetes-Operator-for-PSMONGODB-RN1.15.0.md). |
| [Physical incremental :octicons-link-external-16:](https://docs.percona.com/percona-backup-mongodb/features/incremental-backup.html) | [1.20.0](RN/Kubernetes-Operator-for-PSMONGODB-RN1.20.0.md) | Tech preview | Copies only data changed after the previous backup | You want smaller, faster follow-up backups | Needs a base incremental backup. Base and increments must be taken from the same node. Take a new base backup if a node is down or after a restore. |
| [PVC snapshot](backups-pvc-snapshots.md) (`external`) | [1.23.0](RN/Kubernetes-Operator-for-PSMONGODB-RN1.23.0.md) | Tech preview | PBM prepares the database; the Operator creates CSI volume snapshots of data PVCs | You need the fastest backup and restore for large volumes | No data upload to object storage. No point-in-time recovery or selective restore. Needs the Volume Snapshot API and a `VolumeSnapshotClass`. |
...
See [Known limitations](limitations.md#backups-and-restores) for the full list. The ones that change the backup design are:
